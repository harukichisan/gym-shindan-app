import { bottleneckLabels, gymTypes, gymWeights, questions, traitReasons } from './config.js';
...
export function computeScores(traits) {
  const scores = {};
  const reasons = {};
...
export function buildPlan(traits, answers) {
  let frequency = '週2';
  let duration = '45〜60分';
...
export function buildBottlenecks(traits) {
  const items = [];
  traits.forEach((trait) => {
